...
class BlasSuite extends FunSuite with DistributedSparkSuite {
  ...
  test("acummulateBlocks") {
    ...
    //check if it can be the last element of a chain of matrix ops
    val acc2 = ((drmA + 1) * 1 - 1).aggregateBlocks(0, seqOp, combOp)
