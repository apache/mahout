...
object GPUMMul extends MMBinaryFunc {
  ...
  override def apply(a: Matrix, b: Matrix, r: Option[Matrix]): Matrix = {
	...
	ex.printStackTrace
        var res = MMul(a, b, r)
	println("zsum = " + res.zSum().toString())
	return res
    }
  }
  ...
  private def gpuRWCW(a: Matrix, b: Matrix, r: Option[Matrix] = None): Matrix = {
    ...
    if(b.getFlavor.isDense) {
      log.warn("Dense matrices are not supported in CUDA backend, using JVM instead")
      return MMul(a, b, r)
    } else {
      ...
      val ctx = new Context()
      val cudaA = toCudaCmpMatrix(a, ctx)
      val cudaB = toCudaCmpMatrix(b, ctx)
      val cudaC = prod(cudaA, cudaB, ctx)
      val mxC = fromCudaCmpMatrix(cudaC)

      ms = System.currentTimeMillis() - ms
      log.debug(s"CUDA multiplication time: $ms ms")
      val gpuTrace = mxC.zSum()
      log.debug(s"CUDA trace: $gpuTrace")

      // uncomment code below to verify results against JVM
/*
      ms = System.currentTimeMillis()
      val jvmTrace = MMul(a, b, r).zSum()
      ms = System.currentTimeMillis() - ms
      log.debug(s"JVM multiplication time: $ms ms")
      log.debug(s"JVM trace: $jvmTrace")
*/
      cudaA.close()
      cudaB.close()
      cudaC.close()
...
*/
  ...
  private def gpuSparseRowRWRW(a: Matrix, b: Matrix, r: Option[Matrix] = None): Matrix = {
    ...
    log.warn("Dense matrices are not supported in CUDA backend")
    return MMul(a, b, r)
  }
