...
import org.apache.mahout.h2obindings.{H2OEngine, ops}
...
class RLikeDrmOpsSuite extends FunSuite with Matchers with MahoutLocalContext {
  ...
  test("C = At %*% B , join") {
    ...
    H2OEngine.optimizerRewrite(C) should equal(OpAtB[Int](A, B))
  ...
  test("C = At %*% B , join, String-keyed") {
    ...
    H2OEngine.optimizerRewrite(C) should equal(OpAtB[String](A, B))
  ...
  test("C = A.t %*% A") {
    ...
    H2OEngine.optimizerRewrite(action = AtA) should equal(OpAtA(A))
  ...
  test("A'x") {
    ...
    H2OEngine.optimizerRewrite(drmA.t %*% x) should equal(OpAtx(drmA, x))
