...
trait RegressionTestsSuiteBase extends DistributedMahoutSuite with Matchers {
  ...
  test("OLS w/Ftest and Ttest validation") {
    ...
    val model = new OrdinaryLeastSquares[Int]().fit(drmX, y)
    //println(model.beta.toString)
    //assert(model.se(0) === 2.6878127323908942)

    model.beta.get(4) - 163.17933 should be < epsilon
    model.beta.get(0) - (-1.33627) should be < epsilon
    model.beta.get(1) - (-13.15770) should be < epsilon
    model.beta.get(2) - (-4.15265) should be < epsilon
    model.beta.get(3) - (-5.679908) should be < epsilon

    model.tScore.get(0) - (-0.49715717) should be < epsilon
    model.tScore.get(1) - (-2.43932888) should be < epsilon
    model.tScore.get(2) - (-2.32654000) should be < epsilon
    model.tScore.get(3) - (-3.01022444) should be < epsilon
    model.tScore.get(4) -  3.143183937  should be < epsilon
  ...
  test("OLS without test statistics") {
    ...
    val drmX = drmData(::, 0 until 4)
    val y = drmData(::, 4 until 5)
    val model = new OrdinaryLeastSquares[Int]().fit(drmX, y, 'calcCommonStatistics → false, 'calculateStandardError → false)
    //println(model.beta.toString)
    //assert(model.se(0) === 2.6878127323908942)
