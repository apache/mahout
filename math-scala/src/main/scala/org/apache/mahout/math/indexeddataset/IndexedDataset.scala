...
/**
  * Wrap a DrmLike[C] with bidirectional ID mappings [[com.google.common.collect.BiMap]]so a user specified
  * label or ID can be stored and mapped to and from the Mahout Int ID used internal to Mahout core code.
