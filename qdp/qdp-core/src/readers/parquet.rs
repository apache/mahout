...
use arrow::array::{Array, Float64Array, FixedSizeListArray, ListArray};
...
impl DataReader for ParquetReader {
    fn read_batch(&mut self) -> Result<(Vec<f64>, usize, usize)> {
        let reader = self.reader.take().ok_or_else(|| {
            MahoutError::InvalidInput("Reader already consumed".to_string())
        })?;
...
impl DataReader for ParquetStreamingReader {
    fn read_batch(&mut self) -> Result<(Vec<f64>, usize, usize)> {
        ...
        let sample_size = self.sample_size.ok_or_else(|| {
            MahoutError::Io("No data read from Parquet file".to_string())
        })?;
