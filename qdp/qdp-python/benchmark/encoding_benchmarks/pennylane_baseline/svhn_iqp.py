...
def _run_training_gpu(
    ...
    *,
    ...
    def cost(weights, bias, X_batch, Y_batch):
        preds = torch.stack([model(weights, bias, x) for x in X_batch])
        return torch.mean((Y_batch - preds) ** 2)

    torch.manual_seed(seed)
    weights = (
        (0.01 * torch.randn(num_layers, n_qubits, 3, device=device, dtype=dtype))
        .detach()
        .requires_grad_(True)
    )
    ...
    # Compile (first run)
    t0 = time.perf_counter()
    _ = circuit(weights, feats_train[0])
        ...
        if early_stop_target is not None and (step + 1) % 100 == 0:
            with torch.no_grad():
                pred_test_now = torch.sign(
                    torch.stack([model(weights, bias, x) for x in feats_test])
                ).flatten()
                test_acc_now = (
    ...
    # Metrics
    with torch.no_grad():
        pred_train = torch.sign(
            torch.stack([model(weights, bias, x) for x in feats_train])
        ).flatten()
        pred_test = torch.sign(
            torch.stack([model(weights, bias, x) for x in feats_test])
        ).flatten()
...
def main() -> None:
    ...
    if args.backend == "gpu" and not torch.cuda.is_available():
        raise SystemExit("CUDA is required for --backend gpu.")
    ...
    print(f"  backend={args.backend}, device_id={args.device_id}")
    print()
