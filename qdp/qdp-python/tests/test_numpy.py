...
def test_encode_from_numpy_basic():
    ...
    try:
        ...
        # Verify shape
        assert tensor.shape == (num_samples, sample_size), f"Expected shape {(num_samples, sample_size)}, got {tensor.shape}"
        ...
        assert torch.allclose(norms, torch.ones_like(norms), atol=1e-5), "States should be normalized"
