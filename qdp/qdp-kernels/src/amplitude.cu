...
// Fused Kernel: Computes L2 Norm -> Syncs -> Normalizes
__global__ void fused_amplitude_encode_kernel(
    const double *__restrict__ input,
    cuDoubleComplex *__restrict__ state,
    size_t input_len,
    size_t state_len,
    double *__restrict__ shared_accum_space)
{
    ...
    for (size_t i = idx * 2; i < input_len; i += stride * 2)
    {
        if (i + 1 < input_len)
        {
            double2 v = __ldg(reinterpret_cast<const double2 *>(input + i));
            local_sum += v.x * v.x + v.y * v.y;
        }
        else
        {
            double v = __ldg(input + i);
            local_sum += v * v;
        }
    }
    ...
    if (threadIdx.x == 0)
        atomicAdd(shared_accum_space, block_sum);

    grid.sync(); // GLOBAL BARRIER

    // --- Phase 2: Inverse Norm ---
    if (grid.thread_rank() == 0)
    {
        double total = *shared_accum_space;
        *shared_accum_space = (total > 0.0 && isfinite(total)) ? rsqrt(total) : 0.0;
    }

    grid.sync(); // GLOBAL BARRIER

    // --- Phase 3: Encoding ---
    double inv_norm = *static_cast<volatile double *>(shared_accum_space);

    for (size_t i = idx; i < state_len / 2; i += stride)
    {
        size_t state_idx = i * 2;
        double v1 = 0.0, v2 = 0.0;

        if (state_idx + 1 < input_len)
        {
            double2 v = __ldg(reinterpret_cast<const double2 *>(input + state_idx));
            v1 = v.x;
            v2 = v.y;
        }
        else if (state_idx < input_len)
        {
            v1 = __ldg(input + state_idx);
        }

        state[state_idx] = make_cuDoubleComplex(v1 * inv_norm, 0.0);
        if (state_idx + 1 < state_len)
        {
            state[state_idx + 1] = make_cuDoubleComplex(v2 * inv_norm, 0.0);
        }
    }
}
...
extern "C"
{
    ...
    {
        // 1. Reset accumulator
        cudaError_t err = cudaMemsetAsync(temp_accum_d, 0, sizeof(double), stream);
        if (err != cudaSuccess)
            return err;

        cuDoubleComplex *state_complex_d = static_cast<cuDoubleComplex *>(state_d);

        // 2. Calculate Occupancy for Cooperative Launch
        int devId = 0;
        cudaGetDevice(&devId);
        int numSms = 0;
        ...
        int numThreads = 256;
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocksPerSm, fused_amplitude_encode_kernel, numThreads, 0);

        int gridSize = numSms * numBlocksPerSm;
        void *args[] = {(void *)&input_d, (void *)&state_complex_d, (void *)&input_len, (void *)&state_len, (void *)&temp_accum_d};
