...
/// Launch basis encoding kernel (F32)
///
/// # Arguments
/// * basis_index - The computational basis state index (0 to state_len-1)
/// * state_d - Device pointer to output state vector
...
int launch_basis_encode_f32(
    ...
    basis_encode_kernel_f32<<<gridSize, blockSize, 0, stream>>>(
        basis_index,
        state_complex_d,
        state_len
    );
...
/// Launch batch basis encoding kernel (F32)
///
/// # Arguments
/// * basis_indices_d - Device pointer to array of basis indices (one per sample)
/// * state_batch_d - Device pointer to output batch state vectors
...
/// # Returns
/// CUDA error code (0 = cudaSuccess)
int launch_basis_encode_batch_f32(
    const size_t* basis_indices_d,
    void* state_batch_d,
    size_t num_samples,
    size_t state_len,
    ...
    basis_encode_batch_kernel_f32<<<gridSize, blockSize, 0, stream>>>(
        basis_indices_d,
        state_complex_d,
        num_samples,
        state_len,
