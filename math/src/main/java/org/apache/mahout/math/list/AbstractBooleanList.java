...
public abstract class AbstractBooleanList extends AbstractList<Boolean> {
   ...
   * inclusive.  Shifts any succeeding elements to the left (reduces their index). This call booleanens the list by
   ...
   */
  @Override
  public void replaceFromWith(int from, Collection<Boolean> other) {
    checkRange(from, size());
    Iterator<Boolean> e = other.iterator();
    int index = from;
   ...
   * DatabaseList extends AbstractBooleanList { ... public void removeFromTo(int from,int to) {
  ...
  public List<Boolean> toList() {
    int mySize = size();
    List<Boolean> list = new ArrayList<Boolean>(mySize);
