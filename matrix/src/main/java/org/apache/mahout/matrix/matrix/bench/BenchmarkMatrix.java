...
public class BenchmarkMatrix {
  ...
  protected static void bench_dgemm(String[] args) {
    ...
    int k = 1;
    String[] types = {args[k++]};
    int cpus = Integer.parseInt(args[k++]);
    double minSecs = new Double(args[k++]);
    double[] densities = {new Double(args[k++])};
    boolean transposeA = Boolean.valueOf(args[k++]);
    boolean transposeB = Boolean.valueOf(args[k++]);

    int[] sizes = new int[args.length - k];
    for (int i = 0; k < args.length; k++, i++) {
      sizes[i] = Integer.parseInt(args[k]);
    }


    org.apache.mahout.matrix.matrix.linalg.SmpBlas
        .allocateBlas(cpus, org.apache.mahout.matrix.matrix.linalg.SeqBlas.seqBlas);
  ...
  protected static void bench_pow(String[] args) {
    ...
    double minSecs = new Double(args[k++]);
    double[] densities = {new Double(args[k++])};
    int exponent = Integer.parseInt(args[k++]);
  ...
  protected static Double2DProcedure fun_dgemm(final boolean transposeA, final boolean transposeB) {
    return new Double2DProcedure() {
      ...
      public void apply(org.apache.mahout.matrix.Timer timer) {
        org.apache.mahout.matrix.matrix.linalg.SmpBlas.getSmpBlas().dgemm(transposeA, transposeB, 1, A, B, 0, C);
      }
    ...
    return new Double2DProcedure() {
      ...
      public void apply(org.apache.mahout.matrix.Timer timer) {
        org.apache.mahout.matrix.matrix.linalg.SmpBlas.getSmpBlas()
            .dgemv(transposeA, 1, A, B.viewRow(0), 0, C.viewRow(0));
      }
  ...
  protected static Double2DProcedure funAssignPlusMult() {
    return new Double2DProcedure() {
      ...
      public void apply(org.apache.mahout.matrix.Timer timer) {
        org.apache.mahout.matrix.matrix.linalg.SmpBlas.getSmpBlas()
            .assign(A, B, org.apache.mahout.jet.math.Functions.plusMult(0.5));
      }
  ...
  protected static Double2DProcedure funElementwiseMult() {
    return new Double2DProcedure() {
      ...
      public void apply(org.apache.mahout.matrix.Timer timer) {
        org.apache.mahout.matrix.matrix.linalg.SmpBlas.getSmpBlas()
            .assign(A, org.apache.mahout.jet.math.Functions.mult(0.5));
      }
    };
  }
  ...
  protected static Double2DProcedure funElementwiseMultB() {
    return new Double2DProcedure() {
      ...
      public void apply(org.apache.mahout.matrix.Timer timer) {
        org.apache.mahout.matrix.matrix.linalg.SmpBlas.getSmpBlas()
            .assign(A, B, org.apache.mahout.jet.math.Functions.mult);
      }
    };
  }
  ...
  protected static Double2DProcedure funLUDecompose() {
    return new Double2DProcedure() {
      org.apache.mahout.matrix.matrix.linalg.LUDecompositionQuick lu =
          new org.apache.mahout.matrix.matrix.linalg.LUDecompositionQuick(0);
  ...
  /**
   * Runs the matrix benchmark operations defined in args or in the file specified by args0. To get detailed help on
   * usage type java org.apache.mahout.matrix.matrix.bench.BenchmarkMatrix -help
   */
  public static void main(String[] args) throws IOException {
    ...
    if (!args[0].equals("-file")) { // interactive mode, commands supplied via java class args
      ...
      */
      java.io.BufferedReader reader = new java.io.BufferedReader(new java.io.FileReader(args[1]));
      ...
      while ((token = stream.nextToken()) != StreamTokenizer.TT_EOF) { // while not end of file
        if (token == StreamTokenizer.TT_EOL) { // execute a command line at a time
          ...
          if (!words.isEmpty()) { // ignore emty lines
          ...
          String word;
          org.apache.mahout.matrix.matrix.impl.Former formatter =
              new org.apache.mahout.matrix.matrix.impl.FormerFactory().create("%G");
...
BufferedReader d
  ...
  protected static String usage(String cmd) {
    ...
    if (cmd.equals("dgemv")) {
      usage += "\ttransposeA = false or true\n";
    }
    if (cmd.equals("dgemm")) {
      usage += "\ttransposeA = false or true\n\ttransposeB = false or true\n";
    }
    if (cmd.equals("pow")) {
      usage += "\texponent = the number of times to multiply; e.g. 1000\n";
    }
    usage += "\tsizes = a list of problem sizes; e.g. 100 200 benchmarks squared 100x100 and 200x200 matrices";
    return usage;
  }
}
