...
public class Partitioning {
   ...
   * "column" is the list of "object" values of a given variable (field, dimension). A "matrix" is a list of "objects"
   * (tuples, points). <p> Now, rows (objects, tuples) are partially sorted according to their values in one given
   * variable (dimension). Two entire rows of the matrix are swapped, whenever two column values indicate so. <p> Of
   * course, the column must not be a column of a different matrix. More formally, there must hold: <br> There exists an
   * <tt>i</tt> such that <tt>matrix.viewColumn(i)==column</tt>. <p> Note that arguments are not checked for validity.
   * <p> <b>Example:</b> <table border="1" cellspacing="0"> <tr nowrap> <td valign="top"><tt>8 x 3 matrix:<br> 23, 22,
   * 21<br> 20, 19, 18<br> 17, 16, 15<br> 14, 13, 12<br> 11, 10, 9<br> 8,  7,  6<br> 5,  4,  3<br> 2,  1,  0 </tt></td>
   * <td align="left"> <p><tt>column = matrix.viewColumn(0);<br> a = 0;<br> b = column.size()-1;</tt><tt><br>
   * splitters={5,10,12}<br> c=0; <br> d=splitters.length-1;</tt><tt><br> partition(matrix,column,a,b,splitters,c,d,splitIndexes);<br>
   * ==><br> splitIndexes == {0, 2, 3}</tt></p> </td> <td valign="top"><tt>8 x 3 matrix:<br> 2,  1,  0<br> 5,  4,  3<br>
   * 8,  7, 6<br> 11, 10, 9<br> 23, 22, 21<br> 20, 19, 18<br> 17, 16, 15<br> 14, 13, 12 </tt></td> </tr> </table>
   */
  private static void xPartitionOld(DoubleMatrix2D matrix, DoubleMatrix1D column, int from, int to, double[] splitters,
   ...
   * "object" values of a given variable (field, dimension). A "matrix" is a list of "objects" (tuples, points). <p>
   * Now, rows (objects, tuples) are partially sorted according to their values in one given variable (dimension). Two
   * entire rows of the matrix are swapped, whenever two column values indicate so. <p> Of course, the column must not
   * be a column of a different matrix. More formally, there must hold: <br> There exists an <tt>i</tt> such that
