...
public abstract class AbstractByteList extends AbstractList<Byte> {
   ...
   * inclusive.  Shifts any succeeding elements to the left (reduces their index). This call byteens the list by <tt>(to
   * - from + 1)</tt> elements.
   *
   ...
   */
  @Override
  public void replaceFromWith(int from, Collection<Byte> other) {
    checkRange(from, size());
    Iterator<Byte> e = other.iterator();
    int index = from;
