...
public class OnlineLogisticRegressionTest {
  ...
  public void testClassify() {
    ...
    assertEquals(Math.exp(-1) / (1 + Math.exp(-1) + Math.exp(-2)), v.get(0), 1e-8);
    assertEquals(Math.exp(-2) / (1 + Math.exp(-1) + Math.exp(-2)), v.get(1), 1e-8);
    assertEquals(1 / (1 + Math.exp(-1) + Math.exp(-2)), v.get(2), 1e-8);

    lr.setBeta(0, 1, 1);

    v = lr.classifyFull(new DenseVector(new double[]{1, 1}));
    assertEquals(1.0, v.zSum(), 1e-8);
    assertEquals(Math.exp(0) / (1 + Math.exp(0) + Math.exp(-2)), v.get(0), 1e-3);
    assertEquals(Math.exp(-2) / (1 + Math.exp(0) + Math.exp(-2)), v.get(1), 1e-3);
    assertEquals(1 / (1 + Math.exp(0) + Math.exp(-2)), v.get(2), 1e-3);

    lr.setBeta(1, 1, 3);

    v = lr.classifyFull(new DenseVector(new double[]{1, 1}));
    assertEquals(1.0, v.zSum(), 1e-8);
    assertEquals(Math.exp(0) / (1 + Math.exp(0) + Math.exp(1)), v.get(0), 1e-8);
    assertEquals(Math.exp(1) / (1 + Math.exp(0) + Math.exp(1)), v.get(1), 1e-8);
    assertEquals(1 / (1 + Math.exp(0) + Math.exp(1)), v.get(2), 1e-8);
  }
  ...
  public void testTrain() throws Exception {
    ...
    assertEquals(0, lr.classifyScalar(input).minus(tmp.getColumn(0)).norm(1), 1e-5);
    assertEquals(0, lr.classifyFull(input).getColumn(0).minus(tmp.getColumn(0)).norm(1), 1e-4);
  }
