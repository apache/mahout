...
public class CBayesThetaReducer extends MapReduceBase implements Reducer<Text, DoubleWritable, Text, DoubleWritable> {
  ...
  public void reduce(Text key,
    ...
    while (values.hasNext()) {
      reporter.setStatus("Complementary Bayes Theta Reducer: " + key);
    ...
    if (weight <= 0.0) {
      log.debug("Negative weights Some error Indicated {}=>{}", token, weight);
    }
    reporter.setStatus("Complementary Bayes Theta Reducer: " + key + " => " + weight);
