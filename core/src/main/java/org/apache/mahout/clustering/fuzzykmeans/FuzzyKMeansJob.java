...
public class FuzzyKMeansJob {
   ...
   * @param clustersIn
   *          the directory pathname for initial clusters
   * @param output
   ...
   * @param numMapTasks
   *          the number of maptasks
   * @param doCanopy
   *          does canopy needed for initial clusters
   * @param m
  ...
  public static void runJob(String input,
    ...
    if (doCanopy) {
      CanopyDriver.runJob(input, clustersIn, ManhattanDistanceMeasure.class.getName(), 100.1, 50.1);
      
    }
    // run fuzzy k -means
    FuzzyKMeansDriver.runJob(input, clustersIn, output, measureClass, convergenceDelta, maxIterations,
