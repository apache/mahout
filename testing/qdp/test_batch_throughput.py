...
def test_batch_throughput_opt_correctness(engine, n_qubits, batch_size, enable_zz):
    """
    Test that the batched IQP logic introduced in PR2 (which splits phase
    and transposes for Tensor Cores) yields identical outputs to the theoretical
    ...
    # 3. Validation
    # We use a strict tolerance since both should be deterministic FP64 or properly scaled.
    torch.testing.assert_close(
        actual_state,
        expected_state,
        rtol=1e-10,
        atol=1e-10,
        msg=f"Batch encoding mismatch for N={n_qubits}, Batch={batch_size}, enable_zz={enable_zz}",
    )
