...
import org.apache.mahout.clustering.kmeans.AssumptionFreeKMCMCSeedGenerator;
...
public class DisplayKMeansAFKMCMC extends DisplayClustering {

  DisplayKMeansAFKMCMC() {
    initialize();
  ...
  public static void main(String[] args) throws Exception {
    ...
    boolean useAFK = ! (args.length == 1 && args[0].equals("random"));
    System.out.println("Using seeding: " + (useAFK ? "AFK-MC2" : "Random"));

    RandomUtils.useTestSeed();
    //generateSamples();
    generateSamples(300, 1, 1, 0.5);
    generateSamples(300, 1, 0, 0.5);
    generateSamples(3000, 0, 2, 0.1);
    ...
    int maxIterations = 2;
    if (runClusterer) {
      runSequentialKMeansClusterer(conf, samples, output, measure, numClusters, maxIterations, convergenceDelta, useAFK);
    } else {
      runSequentialKMeansClassifier(conf, samples, output, measure, numClusters, maxIterations, convergenceDelta);
    }
    new DisplayKMeansAFKMCMC();
  }
  ...
  private static void runSequentialKMeansClusterer(Configuration conf, Path samples, Path output,
    DistanceMeasure measure, int numClusters, int maxIterations, double convergenceDelta, boolean useAFK)
    throws IOException, InterruptedException, ClassNotFoundException {
    Path clustersIn = new Path(output, "random-seeds");
    if (useAFK) {
      AssumptionFreeKMCMCSeedGenerator.buildSeed(conf, samples, clustersIn, numClusters, measure);
    } else{
