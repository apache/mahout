...
//    parser.parse(args, Config()) map { config =>
      ...
      val stream = env
        ...
        .map(record => {
          ...
          (key, frame, x.toFloat + w.toFloat / 2, y.toFloat + h.toFloat / 2)
        } ).addSink(new BucketingSink[(String, Int, Float, Float)]("file:///home/rawkintrevo/gits/cylons/data"))


      // First window exercise- Group By Frame Number, Count

      // execute program
      env.execute("Flink Face MetaData Demo")
