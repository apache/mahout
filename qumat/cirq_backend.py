...
def initialize_backend(backend_config: Mapping[str, Any]) -> cirq.Simulator:
...
def apply_swap_gate(
    circuit: cirq.Circuit, qubit_index1: int, qubit_index2: int
) -> None:
...
def apply_t_gate(circuit: cirq.Circuit, qubit_index: int) -> None:
    qubit = cirq.LineQubit(qubit_index)
...
def apply_rx_gate(
    circuit: cirq.Circuit,
    qubit_index: int,
    angle: float | int | str | sympy.Expr,
) -> None:
...
def calculate_prob_zero(
    results: dict[int, int] | list[dict[int, int]],
    ancilla_qubit: int,
    num_qubits: int,
) -> float:
    """
